...
<div class="content-wrapper">
  <section class="content-header">
    <h1>
  <i class="fa fa-lock" aria-hidden="true"></i>&nbsp;Password Member
    </h1>
    <ol class="breadcrumb">
      ...
      <div class="box box-info">
        ...
        <form action="ganti_pass_act.php" method="post" enctype="multipart/form-data">
          <div class="box-body">
              <?php
              $id=$_SESSION['id'];
              $data_member=mysqli_query($koneksi,"select * from tbl_member where id_member='$id' ");
               while($da = mysqli_fetch_array($data_member)){
              ?>                 
            <div class="form-group">
              <label for="inputEmail3" class="control-label">Password baru</label>
                <input type="text" name="id_member" value="<?php echo $id?>" hidden>
                <input type="password" class="form-control" required="required" name="password" placeholder="input disini">
            </div>
            <?php }?>      
          </div>
          <!-- /.box-body -->
